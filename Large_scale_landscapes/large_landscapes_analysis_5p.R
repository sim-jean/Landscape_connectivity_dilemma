################################################################################
####### Script for genetic algorithm analysis of large scale landscapes ########
################################################################################
...
# Goal : 
# 1. Generate real looking landscapes - like 10
# 2. Analyse them with the routine of 5 years and repeated myopic analysis
# 3. Repeat analysis for all biodiverisity levels

# Desired outputs : 
# Geographical outputs
# 1. Initial landscape 
# 2. Succession of treatments
# 3. Conditional on biodiversity level
# Format envisioned : sparseMatrix for efficient storage
...
# General principle : 
# 1. Generate landscapes into a list
# 2. For each landscape : 
# a. Loop over constraint levels the genetic algorithm
# b. May need to parallel process it
...
if(file.exists(paste0(working_path,"/adjacencies/adjacency_", size, ".mtx"))){
...
for(number in 1:20){
  if(runner_== "local"){
    to_open = here(working_path, "Large_scale_landscapes")
  }else{
    to_open = paste0(working_path, '/large_land/input_')
  }
  list_landscape[[number]] = readMM(paste0(to_open,'/landscape_',1,'.mtx'))
}

if(!(file.exists(paste0(working_path,'large_land/output_/result_5peat_0.mtx')))){
  ...
  for(const_ in as.character(seq(0,100,10))){
    ...
    writeMM(to_save, paste0(working_path, 'large_land/output_/result_5peat_',const_, '.mtx'))
  }
...
print(paste("Results took : ", Sys.time() - global_start))
